...
export class SharedService {
  ...
  login(pwd) : Observable<any> {
    return this.http.post(this.APIUrl + '/api/login',pwd, {withCredentials: true},);
 
  }


  get_loggeduser():Observable<any[]>{
  ...
  getPersonnelByService(serv) : Observable<any>{
    return this.http.get(this.APIUrl + '/getPersonnelByService/'+serv);
    
  }  

  getRendement():Observable<any[]>{
    return this.http.get<any[]>(this.APIUrl + '/rendement/');
  }
  addRendement(personnelList) : Observable<any> {
    return this.http.post(this.APIUrl + '/rendement/',JSON.stringify(personnelList),{
      headers : this.header
    });
  }
